...
def create_meeting(date_str, protocol_number):
    with _lock:
        ...
        meeting_id = f"m{int(datetime.now().timestamp())}"
        meetings.append({
            "id": meeting_id,
            "date": date_str,
...
def toggle_attendee(meeting_id, pin):
    """Переключить присутствие. Возвращает True если добавлен, False если убран."""
    with _lock:
        meetings = get_meetings()
        for mt in meetings:
            if mt["id"] == meeting_id:
...
def get_session(meeting_id, session_id):
    mt = get_meeting(meeting_id)
    if mt:
            ...
            if mt["id"] == meeting_id:
                for sess in mt["sessions"]:
                    if sess["id"] == session_id:
                        q_id = f"q{int(datetime.now().timestamp())}{len(sess['questions'])}"
