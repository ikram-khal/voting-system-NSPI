...
async def show_questions(chat_id, ctx, msg_id, mid):
    ...
    if not mt["questions"]:
        text = f"❓ *Вопросы — Заседание №{mt['protocol_number']}*\n\nНет вопросов."
    else:
        lines = [f"❓ *Вопросы — Заседание №{mt['protocol_number']}*\n"]
...
async def handle_admin_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "add_member":
        ...
        await update.message.reply_text(f"✅ Заседание: {date_str}, №{proto}\n/admin")
